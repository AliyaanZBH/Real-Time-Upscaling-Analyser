...
/*
   ...
   [AZB] 24/10/24: Created dedicated ouput buffer for DLSS
   [AZB] 11/11/24: Created dedicated input buffer for DLSS
*/
...
#if AZB_MOD
void Graphics::InitializeRenderingBuffersDLSS(uint32_t bufferWidth, uint32_t bufferHeight, uint32_t dlssWidth, uint32_t dlssHeight)
{
    GraphicsContext& InitContext = GraphicsContext::Begin();
    ...
    // [AZB]: Create buffers for DLSS, using a lower resolution for the input buffer
    g_DLSSInputBuffer.Create(L"DLSS Input Buffer", dlssWidth, dlssHeight, 1, DXGI_FORMAT_R10G10B10A2_UNORM, esram);
    g_DLSSOutputBuffer.Create(L"DLSS Output Buffer", g_DisplayWidth, g_DisplayHeight, 1, DXGI_FORMAT_R10G10B10A2_UNORM, esram);
...
void Graphics::ResizeDLSSInputBuffer(uint32_t newWidth, uint32_t newHeight)
{
    g_DLSSInputBuffer.Create(L"DLSS Input Buffer", newWidth, newHeight, 1, DXGI_FORMAT_R10G10B10A2_UNORM);
